...
function LoginForm() {
  ...
  // here with next=/dashboard/creator, which should preselect the toggle.
  const inferredRole =
    roleParam === "creator" || (nextParam && /\/creator\b/.test(nextParam))
      ? "creator"
      : "brand";
  ...
  // them to whichever dashboard their profile already maps to.
  const next = nextParam || `/dashboard?role=${role}`;
