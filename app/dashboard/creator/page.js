      ...
      <main className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8 max-w-4xl mx-auto space-y-6">
        ...
        {!profileComplete && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-center gap-4">
            <div className="flex-shrink-0 h-10 w-10 rounded-xl bg-white flex items-center justify-center text-amber-500">
              <ImagePlus size={18} />
            </div>
            ...
            <Link
              ...
              className="flex-shrink-0 text-xs font-semibold text-amber-800 hover:underline underline-offset-2 flex items-center gap-1"
            >
              Complete profile
              <ArrowRight size={12} />
            </Link>
        ...
        {(loading || actionNeeded.length > 0) && (
          <section>
            <h2 className="text-sm font-semibold text-brand-ink mb-3 flex items-center gap-2">
              <span className="h-2 w-2 rounded-full bg-amber-400 animate-pulse" />
              Action needed
              {actionNeeded.length > 0 && (
                <span className="text-xs font-medium text-slate-400">({actionNeeded.length})</span>
              )}
            </h2>
            {loading ? (
        ...
        {inProgress.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-brand-ink mb-3 flex items-center gap-2">
              <Clock size={14} className="text-brand-skyDeep" />
              Waiting on brand review
              <span className="text-xs font-medium text-slate-400">({inProgress.length})</span>
            </h2>
            <div className="space-y-2">
              {inProgress.map((a) => (
        ...
        {showEmpty && (
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center">
            <div className="mx-auto h-14 w-14 rounded-2xl bg-brand-mist flex items-center justify-center mb-4">
              <Upload size={22} className="text-brand-skyDeep" />
            </div>
            <p className="text-base font-semibold text-brand-ink">No assignments yet</p>
            ...
            <Link
              href="/dashboard/creator/profile/edit"
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-brand-ink text-white px-5 py-2.5 text-sm font-semibold hover:bg-slate-800 transition"
            >
        ...
        {hasAnyAssignments && (
          <div className="grid grid-cols-3 gap-3">
            ...
            <StatCard
              ...
              icon={<Eye size={16} />}
              tint="text-brand-skyDeep bg-brand-mist"
            />
            <StatCard
              label="Approved"
        ...
        {hasAnyAssignments && (
          <div className="text-center">
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-skyDeep hover:underline underline-offset-2"
            >
...
function AssignmentRow({ assignment, variant }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 hover:border-brand-sky/60 hover:shadow-sm transition"
    >
      <div className={`h-10 w-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
        variant === "action" ? "bg-amber-50 text-amber-600" : "bg-brand-mist text-brand-skyDeep"
      }`}>
        {variant === "action" ? <Upload size={18} /> : <Eye size={18} />}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-brand-ink truncate">{assignment.title}</p>
...
function StatCard({ label, value, icon, tint }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <p className="text-2xl font-semibold text-brand-ink tabular-nums">{value}</p>
