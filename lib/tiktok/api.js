...
// Paginated list of the creator's own posted videos. `cover_image_url` is a
// signed CDN URL that expires, so callers must rewrite it on every sync.
export async function fetchVideoList(accessToken, cursor = 0) {
  const res = await fetch(
    `${VIDEO_LIST_URL}?fields=id,title,video_description,duration,cover_image_url,create_time,share_url,view_count,like_count,comment_count,share_count`,
    {
      method: "POST",
      headers: {
