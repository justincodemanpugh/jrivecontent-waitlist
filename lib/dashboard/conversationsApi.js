  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, gig_id, brand_id, creator_id, last_message_at, created_at, payment_deposited,
       ...
       payment:payments ( id, status, amount_cents, platform_fee_cents, creator_payout_cents, videos_paid_out ),
       ${counterpartJoin}`,
    )
    .eq("id", id)
    .eq(ownershipCol, user.id)
...
// Lets the message thread refresh `payment_deposited`, `videos_completed`,
// etc. live instead of after a page reload.
