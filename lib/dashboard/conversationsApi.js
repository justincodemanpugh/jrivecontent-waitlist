...
// Each conversation links a brand + creator on either a gig or a program.
...
export async function fetchMyConversations(role) {
  ...
  let query = supabase
    .from("conversations")
    .select(
      `id, gig_id, program_id, brand_id, creator_id, last_message_at, created_at,
       gig:gigs ( id, title, pay_per_video ),
       program:programs ( id, title, pay_per_video_cents ),
       ${counterpartJoin}`,
    )
  ...
  return (data || []).map((row) => ({
    ...
    subjectTitle: row.program?.title || row.gig?.title || "Conversation",
    payPerVideo: row.program
      ? Number(row.program.pay_per_video_cents || 0) / 100
      : Number(row.gig?.pay_per_video) || 0,
  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, gig_id, program_id, brand_id, creator_id, last_message_at, created_at,
       ...
       program:programs ( id, title, description, pay_per_video_cents ),
       ${counterpartJoin}`,
    )
    .eq("id", id)
    .eq(ownershipCol, user.id)
