...
export async function fetchDeliverable(id) {
  ...
  if (userErr || !user) throw new Error("You need to be signed in to view this deliverable.");

  // Defense-in-depth: explicitly require the current user to be the
  // brand or creator on this deliverable. RLS already enforces the same
  // rule, but this surfaces a clear "not found" error instead of letting
  // the caller think it succeeded with empty data.
...
export async function approveDeliverable(id) {
  ...
  // NOTE: The conversation's `videos_completed` counter is bumped server-side
  // by /api/stripe/release (admin client, no RLS), since brands don't have
  // an RLS update policy on the conversations table.
}
