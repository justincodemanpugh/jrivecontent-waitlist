...
export async function updateCreatorProfile(payload) {
  ...
  // If the TikTok handle changed here too, reset the fallback sync status so the
  // creator's Campaigns page doesn't keep showing counts for the old handle.
  // (setCreatorTikTokHandle does the same for the dedicated fallback field.)
  const { data: current } = await supabase
    .from("creator_profiles")
    .select("tiktok_handle")
    .eq("user_id", user.id)
    .maybeSingle();
  if ((current?.tiktok_handle || "") !== update.tiktok_handle) {
    if (update.tiktok_handle) {
      update.tiktok_handle_sync_status = "pending";
      update.tiktok_handle_synced_at = null;
      update.tiktok_handle_sync_error = null;
    } else {
      update.tiktok_handle_synced_at = null;
      update.tiktok_handle_sync_status = null;
      update.tiktok_handle_sync_error = null;
      update.tiktok_handle_video_count = null;
    }
  }
...
export async function setCreatorTikTokHandle(handle) {
  ...
  // Detect a real change so we can reset the sync-status columns: a new handle
  // must be re-checked immediately, so clear the cooldown anchor
  // (tiktok_handle_synced_at) that app/api/programs/member-sync reads.
  const { data: current } = await supabase
    .from("creator_profiles")
    .select("tiktok_handle")
    .eq("user_id", user.id)
    .maybeSingle();
  const changed = (current?.tiktok_handle || "") !== username;

  const update = { tiktok_handle: username };
  if (!username) {
    // Cleared — wipe the fallback sync status entirely.
    update.tiktok_handle_synced_at = null;
    update.tiktok_handle_sync_status = null;
    update.tiktok_handle_sync_error = null;
    update.tiktok_handle_video_count = null;
