...
// Single target platform a brief is for. Optional but recommended.
...
export async function fetchMyBriefs() {
  ...
  const { data, error } = await supabase
    .from("briefs")
    .select(`
      id, title, instructions, pay_per_creator, platform, status, created_at, updated_at,
      brief_recipients (
...
export async function fetchBriefById(briefId) {
  ...
  const { data, error } = await supabase
    .from("briefs")
    .select(`
      id, title, instructions, pay_per_creator, platform, status, created_at, updated_at, brand_id,
      brief_recipients (
        id, creator_id, status, sent_at, viewed_at,
