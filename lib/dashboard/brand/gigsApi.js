...
// Free-tier: brands can browse creators freely but must start a trial to post gigs.
// Setting FREE_GIGS_TOTAL to 0 means free users cannot post any gigs.
export const FREE_GIGS_TOTAL = 0;
...
// is on the Pro plan (including the 3-day trial) and should bypass
// free-tier gates. Delegates to lib/billing/subscription so the list of
// paid statuses lives in exactly one place.
export async function isBrandPro(supabase, userId) {
  return brandHasActiveSubscription(supabase, userId);
}
...
export async function publishGig(form) {
  ...
  // Free-tier gate: require trial/subscription to post gigs.
  // Free users can browse creators but must start a 3-day trial to post.
  if (!(await isBrandPro(supabase, user.id))) {
    throw new Error(
      "Start your free 3-day trial to post gigs and connect with creators.",
    );
  }
