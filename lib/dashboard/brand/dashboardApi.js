...
//   pendingSubmissions:  brief submissions awaiting review
//   completedThisMonth:  submissions completed this calendar month
export async function fetchDashboardStats() {
  ...
  // Try to fetch from briefs tables, fall back to 0 if tables don't exist yet
  let activeBriefs = 0;
  let connectedCreators = 0;
  let pendingSubmissions = 0;
  let completedThisMonth = 0;

  try {
    const [briefsRes, creatorsRes, pendingRes, completedRes] = await Promise.all([
      supabase
        .from("briefs")
    ...
    activeBriefs = briefsRes.count ?? 0;
    connectedCreators = creatorsRes.count ?? 0;
