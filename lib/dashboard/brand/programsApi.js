...
// Turn the flat list of tracked videos (from fetchProgramVideos) into a daily
// time series + trailing-window deltas for the Overview chart and the "+X this
// period" stat badges. Pure/synchronous so it's easy to reason about and test.
//
// The view curve is approximated by attributing each video's current view
// count to the day it was posted and cumulating — exact per-day history would
// need the metric snapshots, but this renders a faithful growth curve and is
// robust when there's little or no data yet.
export function buildProgramMetrics(videos, { days = 30 } = {}) {
  ...
  const buckets = Array.from({ length: days }, (_, i) => {
    ...
    return { date: d.toISOString(), addViews: 0, addVideos: 0 };
  });
  ...
  let cumViews = totalViews - deltas.views;
  let cumVideos = (videos || []).length - deltas.postedVideos;

  const series = buckets.map((b) => {
    cumViews += b.addViews;
    cumVideos += b.addVideos;
    return { date: b.date, views: cumViews, postedVideos: cumVideos };
  });
