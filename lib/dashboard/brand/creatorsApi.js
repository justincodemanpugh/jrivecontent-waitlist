...
// ---------------------------------------------------------------------------
// Brand Creators Roster (My Creators)
// ---------------------------------------------------------------------------

function notifyBrandCreatorsChanged() {
  if (typeof window !== "undefined") {
    window.dispatchEvent(new Event("brand-creators:changed"));
  }
}

// Fetch the brand's connected creators roster.
export async function fetchMyCreators() {
  const supabase = createClient();
  const {
    data: { user },
    error: userErr,
  ...
  const { data, error } = await supabase
    .from("brand_creators")
    .select(`
      id, status, notes, added_at, updated_at,
      creator_id,
      creator_profiles!brand_creators_creator_id_fkey_profile (
        user_id, display_name, handle, bio, avatar_url, cover_photo_url,
        niches, content_types, location, rate_min, rate_max,
  ...
  return (data || []).map((row) => {
    ...
    return {
      ...
      location: p?.location || "",
      rateMin: p?.rate_min || null,
      rateMax: p?.rate_max || null,
...
// Roster errors reach the user through a bare alert() in CreatorsView, so a
// rethrown PostgrestError puts raw SQL on screen — which is exactly how
// "violates foreign key constraint brand_creators_brand_id_fkey_profile"
// became a user-facing message. Translate the codes we understand and keep
// everything else generic, logging the real error for debugging.
function rosterError(error, fallback) {
  console.error("[brand_creators]", error);
  switch (error?.code) {
    case "23505": // unique_violation
      return new Error("You've already connected with this creator.");
    case "23503": // foreign_key_violation
      return new Error(
        "Finish setting up your brand profile before connecting with creators.",
      );
...
// Send a connection request to a creator.
export async function connectWithCreator(creatorId, notes = "") {
  const supabase = createClient();
  const {
    data: { user },
    error: userErr,
    ...
    .upsert(
      {
        brand_id: user.id,
        creator_id: creatorId,
        // A brand's roster is private and self-managed: a creator the brand
        // adds is immediately active so the brand can send them briefs. There
        // is no separate creator-side acceptance step for brand_creators.
        status: "active",
  ...
  if (error) throw rosterError(error, "Couldn't connect with this creator.");

  notifyBrandCreatorsChanged();
  return data;
}

// Remove a creator from the brand's roster.
export async function removeCreatorFromRoster(connectionId) {
  const supabase = createClient();
  const { error } = await supabase
    .from("brand_creators")
    .delete()
    .eq("id", connectionId);
  if (error) throw rosterError(error, "Couldn't remove this creator.");
  notifyBrandCreatorsChanged();
}

// Update notes for a creator in the roster.
export async function updateCreatorNotes(connectionId, notes) {
  const supabase = createClient();
  const { error } = await supabase
    .from("brand_creators")
...
// Fetch connection status for multiple creators (used by browse grid).
export async function fetchConnectionStatusForCreators(creatorIds) {
  if (!creatorIds || creatorIds.length === 0) return [];
  const supabase = createClient();
  const { data, error } = await supabase
    .from("brand_creators")
