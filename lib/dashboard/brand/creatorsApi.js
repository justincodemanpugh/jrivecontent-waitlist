...
import { isBrandPro, FREE_INVITES_TOTAL } from "@/lib/dashboard/brand/gigsApi";
...
export async function fetchAllCreators() {
  ...
  if (ids.length > 0) {
    const { data: vids, error: vErr } = await supabase
      ...
      .select("id, creator_id, storage_path, position")
      .in("creator_id", ids)
...
export async function inviteCreatorToGig({ creatorId, gigId, message }) {
  ...
  // Free-tier gate: brands on the free plan can only ever send 1 invite.
  // Re-inviting the same (gig, creator) pair is still allowed because
  // the upsert below doesn't create a new row.
  if (!(await isBrandPro(supabase, user.id))) {
    ...
    if (!existing) {
      const { count } = await supabase
        ...
        .eq("brand_id", user.id);
      if ((count ?? 0) >= FREE_INVITES_TOTAL) {
        throw new Error(
          "You've used your free invite. Upgrade to Pro to invite more creators.",
        );
      }
    }
  }
