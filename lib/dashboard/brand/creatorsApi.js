...
export async function fetchAllCreators() {
  ...
  if (ids.length > 0) {
    const { data: vids, error: vErr } = await supabase
      ...
      .select("id, creator_id, platform, video_url, thumbnail_url, thumbnail_path, title, is_featured, created_at")
      .in("creator_id", ids)
      .order("created_at", { ascending: false });
    if (vErr) throw vErr;
    for (const v of vids || []) {
...
export async function inviteCreatorToGig({ creatorId, gigId, message }) {
  ...
  // Only Pro brands can invite creators
  if (!(await isBrandPro(supabase, user.id))) {
    throw new Error(
      "Upgrade to Pro to invite creators.",
    );
  }
