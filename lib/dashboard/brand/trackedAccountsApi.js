...
// Force an immediate sync of everything this brand tracks, rather than waiting
// for the nightly cron. Rate-limited server-side.
export async function refreshTrackedMetrics() {
  const res = await fetch("/api/programs/refresh", { method: "POST" });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || "Could not refresh metrics.");
  notifyTrackedAccountsChanged();
  return json;
}
